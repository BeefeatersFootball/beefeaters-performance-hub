...
async function getInjuryContent() {
  const videosQuery = `*[_type == "videoResource" && category == "injury"] | order(_createdAt desc){ _id, title, youtubeUrl }`;
  const pdfsQuery = `*[_type == "pdfResource" && category == "injury"] | order(_createdAt desc){ _id, title, "fileUrl": file.asset->url }`;
...
export default async function InjuryPage() {
  const { videos, pdfs } = await getInjuryContent();
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-[#0B1F4B] to-black text-white p-6">
      ...
      <header className="mb-10">
        <h1 className="text-3xl font-bold tracking-tight">Injury Prevention</h1>
        <p className="text-gray-300 mt-2 max-w-md">
          Strategies to stay healthy, resilient, and available throughout the season.
        </p>
      </header>
      ...
      <section className="mb-10">
        ...
        {videos.length === 0 ? (
          <p className="text-gray-400">No videos yet. Add category <span className="text-white font-medium">injury</span> in Sanity.</p>
        ) : (
      ...
      <section>
        ...
        {pdfs.length === 0 ? (
          <p className="text-gray-400">No PDFs yet. Add category <span className="text-white font-medium">injury</span> in Sanity.</p>
        ) : (
