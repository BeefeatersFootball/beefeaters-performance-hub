...
async function getBrainContent() {
  const videosQuery = `*[_type == "videoResource" && category == "brain"] | order(_createdAt desc){ _id, title, youtubeUrl }`;
  const pdfsQuery = `*[_type == "pdfResource" && category == "brain"] | order(_createdAt desc){ _id, title, "fileUrl": file.asset->url }`;
...
export default async function BrainPage() {
  const { videos, pdfs } = await getBrainContent();
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-[#0B1F4B] to-black text-white p-6">
      ...
      <header className="mb-10">
        <h1 className="text-3xl font-bold tracking-tight">Brain Performance & Concussion</h1>
        <p className="text-gray-300 mt-2 max-w-md">
          Resources supporting brain health, concussion recovery, and cognitive performance.
        </p>
      </header>
      ...
      <section className="mb-10">
        ...
        {videos.length === 0 ? (
          <p className="text-gray-400">No videos yet. Add category <span className="text-white font-medium">brain</span> in Sanity.</p>
        ) : (
      ...
      <section>
        ...
        {pdfs.length === 0 ? (
          <p className="text-gray-400">No PDFs yet. Add category <span className="text-white font-medium">brain</span> in Sanity.</p>
        ) : (
