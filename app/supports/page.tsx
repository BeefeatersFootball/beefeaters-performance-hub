...
async function getSupportsContent() {
  const videosQuery = `*[_type == "videoResource" && category == "supports"] | order(_createdAt desc){ _id, title, youtubeUrl }`;
  const pdfsQuery = `*[_type == "pdfResource" && category == "supports"] | order(_createdAt desc){ _id, title, "fileUrl": file.asset->url }`;
...
export default async function SupportsPage() {
  const { videos, pdfs } = await getSupportsContent();
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-[#0B1F4B] to-black text-white p-6">
      ...
      <header className="mb-10">
        <h1 className="text-3xl font-bold tracking-tight">Additional Supports</h1>
        <p className="text-gray-300 mt-2 max-w-md">
          Resources supporting physical, mental, and social well-being.
        </p>
      </header>
      ...
      <section className="mb-10">
        ...
        {videos.length === 0 ? (
          <p className="text-gray-400">No videos yet. Add category <span className="text-white font-medium">supports</span> in Sanity.</p>
        ) : (
      ...
      <section>
        ...
        {pdfs.length === 0 ? (
          <p className="text-gray-400">No PDFs yet. Add category <span className="text-white font-medium">supports</span> in Sanity.</p>
        ) : (
